...
export default function QueuePage() {
  ...
  useEffect(() => {
    ...
    s.on('connect', () => {
      ...
      // Registrar el usuario si ya está logueado
      if (user) {
    ...
    s.on('queue_position', (data: { position: number; estimatedWait: number }) => {
      ...
      addLog(`📍 Posición en cola: #${data.position} — Espera: ${data.estimatedWait < 60 ? `${data.estimatedWait}s` : `${Math.floor(data.estimatedWait / 60)} min`}`);
    });
    ...
    s.on('your_turn', (data: { message: string; timeoutSeconds: number }) => {
      setYourTurn(true);
      setInQueue(false);
      setMyPosition(null);
      setEstimatedWait(null);
      setTurnTimeout(data.timeoutSeconds);
      addLog(`🎯 ¡ES TU TURNO! Tenés ${data.timeoutSeconds} segundos para comprar.`);

      // Countdown del turno
      let remaining = data.timeoutSeconds;
      const interval = setInterval(() => {
        remaining--;
        setTurnTimeout(remaining);
        if (remaining <= 0) {
          clearInterval(interval);
          setYourTurn(false);
          setTurnTimeout(null);
          addLog('⏰ Se acabó el tiempo. Tu turno expiró.');
        }
      }, 1000);
    });
    ...
    <div className="max-w-5xl mx-auto py-8 sm:py-12">
      ...
      <div className="mb-10">
        ...
        <p className="text-muted text-base max-w-xl">
          Demostrá el sistema de cola virtual bajo alta demanda. Simulá hasta 100.000 usuarios y observá el despacho automático en tiempo real.
        </p>
      </div>

      {/* Banner turno */}
      {yourTurn && (
        <div className="mb-6 bg-amber/10 border border-amber/50 rounded-2xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 animate-fade-in-up">
          <div>
            <p className="text-amber font-display text-2xl font-black uppercase mb-1">
              🎯 ¡Es tu turno!
            </p>
            <p className="text-muted text-sm">
              Tenés <span className="text-white font-bold">{turnTimeout}s</span> para completar tu compra.
            </p>
          </div>
          <button
            onClick={() => navigate('/events')}
            className="px-6 py-3 bg-amber hover:bg-amber-dark text-black rounded-lg text-sm font-bold uppercase tracking-widest transition-all whitespace-nowrap"
          >
            Ir a comprar →
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-4">
              ...
              <div className="bg-surface/80 border border-border/30 rounded-xl p-4 col-span-2">
                ...
                <p className="font-display text-3xl font-black text-white">
                  {estimatedWait
                    ? estimatedWait < 60
                      ? `${estimatedWait}s`
                      : `${Math.floor(estimatedWait / 60)} min`
                    : '—'}
                </p>
              </div>
            </div>
          ...
          <div className="bg-surface-2/50 border border-border/50 rounded-2xl p-6">
            ...
            {!isAuthenticated ? (
              ...
              <p className="text-amber text-sm font-semibold">
                ¡Estás siendo atendido! Usá el botón de arriba para ir a comprar.
              </p>
            ) : (
          ...
          <div className="bg-surface-2/50 border border-border/50 rounded-2xl p-6">
            ...
            <p className="text-muted text-xs mb-5">
              Generá usuarios virtuales. El sistema despacha automáticamente cada 10 segundos.
            </p>
            ...
            <div className="flex gap-2 mb-3">
              ...
              />
              <button
                onClick={() => { const n = parseInt(customCount); if (n > 0) handleSimulate(n); }}
        ...
        <div className="bg-surface-2/50 border border-border/50 rounded-2xl p-6 flex flex-col">
          ...
          <div ref={logRef} className="flex-1 overflow-y-auto space-y-2 min-h-[400px] max-h-[500px]">
            {log.length === 0 ? (
              <p className="text-muted text-xs text-center py-8">Los eventos aparecerán acá...</p>
            ) : (
              log.map((entry, i) => (
